...
export class AdminDashboardComponent {
  ...
  getAllSpaces() {
    this.masterService.getAllSpace().subscribe((res: any) => {
      this.spaces = res;
    });
  }

  getAllFinishes() {
    this.masterService.getAllFinish().subscribe((res: any) => {
      this.finishes = res;
    });
  }
  ...
  getAllDesigns() {
    this.masterService.getAllDesign().subscribe((res: any) => {
      this.designs = res;
    });
  }
  ...
  getAllBrands(): Observable<any> {
    return this.masterService.getAllBrand();
  }
  ...
  getProductDetails(productCode: any) {
    this.productService
      ...
      .subscribe((data: any) => {
        ...
        this.productPayload.name = data[0].name;
        this.productPayload.description = data[0].description;
        this.productPayload.colorId = data[0].color[0]?.id || 0;
        this.productPayload.materialId = data[0].material[0]?.id || 0;
        this.productPayload.finishId = data[0].finish[0]?.id || 0;
        this.productPayload.spaceId = data[0].space[0]?.id || 0;
        this.productPayload.roomId = data[0].room[0]?.id || 0;
        this.productPayload.priceRangeId = data[0].priceRange[0]?.id || 0;
        this.productPayload.price = data[0].price || 0;
        this.productPayload.discountId = data[0].discount[0]?.id || null;
        this.productPayload.inStock = data[0].inStock;
        this.productPayload.isNewLaunch = data[0].isNewLaunch;
        this.productPayload.productCode = data[0].productCode;
        this.productPayload.tagList = data[0]?.tagList || [];
        this.productPayload.categoryId = data[0].category[0]?.id || 0;
        ...
        this.productPayload.productSizeMappingsList =
          data[0].productSizeList.map((mapping: any) => mapping.id);
        if (data[0].brand && data[0].brand.length > 0) {
          this.productPayload.brandId = data[0].brand[0].id;

          this.brandCtrl.setValue(data[0].brand[0]);
        }
      });
  }
  ...
  onBrandSelected(event: MatAutocompleteSelectedEvent) {
    ...
    this.productPayload.brandId = selectedBrand.id;
  }
  ...
  loadInitialPayload() {
    ...
    this.payload = {
      ...
      spaceId: this.productPayload.spaceId,
      brandId: this.productPayload.brandId,
      finishId: this.productPayload.finishId,
      categoryId: this.productPayload.categoryId,
      designId: this.productPayload.designId,
  ...
  validatePostForm(payload: any): boolean {
    ...
    } else if (!payload.discountId) {
      this.showNotification("Discount is required");
    } else if (!payload.spaceId) {
      this.showNotification("Space is required");
    } else if (!payload.categoryId) {
      this.showNotification("Category is required");
    } else if (!payload.finishId) {
      this.showNotification("Finish is required");
    } else if (!payload.brandId) {
      this.showNotification("Brand is required");
    } else if (!payload.designId) {
      this.showNotification("Design is required");
